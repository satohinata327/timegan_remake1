...
def tail_quantile_loss(
    ...
    x_real_flat = x_real.reshape(-1, feature_dim)
    x_fake_flat = x_fake.reshape(-1, feature_dim)
    real_q = torch.quantile(x_real_flat, quantiles, dim=0).detach()
    fake_q = torch.quantile(x_fake_flat, quantiles, dim=0)
    scaled_diff = (fake_q - real_q) / feature_std
    return torch.mean(scaled_diff * scaled_diff)
...
def train(config: dict) -> None:
    ...
    tail_quantile_weight = float(config.get("tail_quantile_loss_weight", 1.0))
    tail_quantile_values = [float(q) for q in config.get("tail_quantiles", [0.05, 0.95])]
    tail_quantiles = torch.tensor(tail_quantile_values, dtype=torch.float32, device=device)
    ...
    save_json(
        dirs["data"] / "training_data_info.json",
        {
            ...
            "tail_quantile_loss_weight": tail_quantile_weight,
            "tail_quantiles": tail_quantile_values,
            "tail_quantile_training_note": (
                "Quantiles are computed on inverse-scaled returns flattened across "
                "the current mini-batch and normalized by each feature's training std."
            ),
    ...
    with log_path.open("w", encoding="utf-8") as log:
        ...
        write("Stage 3/3: joint adversarial training with abs autocorr, correlation, and tail losses")
        seq_len = int(config["seq_len"])
        noise_dim = int(config["noise_dim"])
        ...
        latest_tail_loss = torch.zeros((), device=device)
        for step in range(1, int(config["joint_iterations"]) + 1):
            ...
            for _ in range(2):
                ...
                latest_tail_loss = tail_quantile_loss(
                    x_real_original,
                    x_fake_original,
                    tail_quantiles,
                    feature_std,
                ...
                g_loss = (
                    ...
                    + tail_quantile_weight * latest_tail_loss
                )
            ...
            if step == 1 or step % 100 == 0:
                write(
                    ...
                    f"tail_loss={float(latest_tail_loss.detach().cpu()):.6f} "
    ...
    checkpoint_path = dirs["models"] / "timegan_abs_ac_corr_tail.pt"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default="timegan_remake1/config/timegan_seq60_abs_ac_corr_tail5.json")
