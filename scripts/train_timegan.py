...
def abs_autocorr_values(series: torch.Tensor, lags: list[int]) -> torch.Tensor:
    ...
    for lag in lags:
        if lag >= seq_len:
            continue
        values.append(batch_corr(abs_series[:, :-lag], abs_series[:, lag:]))
    if not values:
...
def abs_autocorr_loss(
    x_real_scaled: torch.Tensor,
    x_fake_scaled: torch.Tensor,
    min_vals: torch.Tensor,
    max_vals: torch.Tensor,
    lags: list[int],
) -> torch.Tensor:
    x_real = inverse_minmax_torch(x_real_scaled, min_vals, max_vals)
    x_fake = inverse_minmax_torch(x_fake_scaled, min_vals, max_vals)
    losses = []
    ...
    if not losses:
        return torch.zeros((), device=x_real_scaled.device)
...
def train(config: dict) -> None:
    ...
    with log_path.open("w", encoding="utf-8") as log:
        ...
        write("Stage 3/3: joint adversarial training with abs autocorr loss")
        seq_len = int(config["seq_len"])
        noise_dim = int(config["noise_dim"])
    ...
    checkpoint_path = dirs["models"] / "timegan_abs_autocorr.pt"
